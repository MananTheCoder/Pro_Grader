...
def run_code(user_code, input_file):
    ...
    with open(input_file, 'r') as infile:
        try:
            result = subprocess.run('./temp_executable', stdin=infile, capture_output=True, text=True, timeout=5)
        ...
        for i, (input_file, expected_output_file) in enumerate(test_cases):
            ...
            if os.path.exists(code_file):
                ...
                if output is not None:
                    current_output, expected_output = compare_output(output, expected_output_file)
